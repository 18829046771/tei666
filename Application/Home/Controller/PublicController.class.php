    	...
    	if(strstr($_SERVER['HTTP_USER_AGENT'],'MicroMessenger')){
	    	...
	    	if(!empty($weixinConfig) ){
	    		...
	    		if($data['status'] == 1){
	    			...
	    			$userOB=M('user');
	    			//发放积分
	    			$fen=$userOB->where(array('id'=>$tt['id']))->find();
	    			$fen_yuan=$fen['jifen'];
	    			$lost_time=$fen['last_login'];
	    			$ktime=strtotime(date("Y-m-d"),time());
	    			if($lost_time<$ktime){
	    				$jifen=$fen_yuan+5;
	    				 $userOB->where(array('id'=>$tt['id']))->setField(array('jifen'=>$jifen));
	    			}
	    			if(isset($_GET['id'])){
    	      	       $userOB->where(array('id'=>$tt['id']))->setField(array('mendian'=>$_GET['id']));
		    	      }
	    			
	    			
	    			
	    			//更新登录时间
	    			$userOB->where(array('id'=>$tt['id']))->setField(array('last_login'=>time()));
...
//  		//会员注册送优惠券
//  		$coupon = M('coupon')->where("send_end_time > ".time()." and ((coupon_createnum - coupon_sendnum) > 0 or coupon_createnum = 0) and coupon_type = 2")->select();
//  		foreach ($coupon as $key => $val)
//  		{
//  			// 送券
//  			M('coupon_uselist')->add(array('coupon_use_cid'=>$val['coupon_id'],'coupon_use_type'=>$val['coupon_type'],'coupon_use_uid'=>$user[user_id],'coupon_use_sendtime'=>time()));
//  			M('Coupon')->where("coupon_id = {$val['coupon_id']}")->setInc('coupon_sendnum'); // 优惠券领取数量加一
//  		}
...
//  		echo '<br/>';
    ...
    public function hezuo(){
    	//只是车管家的合作企业  
    	$where['cate_id']=array('neq', 22);
    	$temp=M("gongying")->where($where)->group('photo')->select();
    	foreach($temp as $k=>$v){
    		 $store=M("article")->where("gong='".$v['id']."'")->select();
    		 $temp[$k]['store']=$store;
    	}
