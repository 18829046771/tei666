...
class CheController extends PublicController {
    ...
    public function cgj(){
    	 //车轮博图
    	$tu=M("picture")->where("id=27||(id=28)||(id=29)||(id=30)||(id=31)")->select();
    	$this->tu1=$tu[0];
    	$this->tu2=$tu[1];
    	$this->tu3=$tu[2];
    	$this->tu4=$tu[3];
    	...
    	$che=M("cate")->where("cate_p_id=18")->select();
...
// 		$this->display('Che/cpxq_ply');
   ...
   public function addcl(){
    	...
    	if(M("ping")->add($arr)){
    		// 结束订单
    		$jieshu=M("dingdan")->where("id='".$dingid."'")->save([
                'ster' => 4,
    		...
    		if($cate['cate_p_id']=='18'){ 
    			//车管家
    			$this->redirect('Che/cpxq_pl', array('id' => $data['art_id']));
    ...
    public function odersheng(){
    	...
    	if($res){
    		$this->redirect('Che/oderitem', array('did' => $res));
    	}
